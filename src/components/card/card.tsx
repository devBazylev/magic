...
function Card({id, name, alt, price, tag, image, isFav, handleAddToCart, handleFav}: CardProps & {
  ...
  return (
    <li className="info__item" data-id={id} data-tag={tag}>
      ...
      <button className={`btn info__fav ${isFav ? 'info__fav--active' : ''}`} type="button" onClick={handleFav} aria-label="Favorite.">
        <svg className="info__heart" fill="transparent" height="200" width="200" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
